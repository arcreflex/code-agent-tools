...
async function ensureShelfAndWorktree(args: {
  ...
  const script = [
    ...
    '[ -d /repo-shelf/repo/.git ] || ( git clone --no-checkout "$HOST_URL" /repo-shelf/repo || git clone --no-checkout "$ORIGIN_URL" /repo-shelf/repo )',
    'git -C /repo-shelf/repo remote add host "$HOST_URL" || git -C /repo-shelf/repo remote set-url host "$HOST_URL"',
    'git -C /repo-shelf/repo remote set-url origin "$ORIGIN_URL" || git -C /repo-shelf/repo remote add origin "$ORIGIN_URL"',
    "git -C /repo-shelf/repo fetch --prune host",
    "git -C /repo-shelf/repo remote get-url origin >/dev/null 2>&1 && git -C /repo-shelf/repo fetch --prune origin",
    ...
    "elif git -C /repo-shelf/repo rev-parse --verify --quiet refs/remotes/origin/$BRANCH_NAME; then",
    ...
    '  [ -n "$DEFAULT_BRANCH" ] || DEFAULT_BRANCH=$(git -C /repo-shelf/repo symbolic-ref --short -q refs/remotes/origin/HEAD | sed "s#^origin/##" || true)',
    '  [ -n "$DEFAULT_BRANCH" ] || DEFAULT_BRANCH=main',
    "  if git -C /repo-shelf/repo rev-parse --verify --quiet refs/remotes/host/$DEFAULT_BRANCH; then",
    '    git -C /repo-shelf/repo worktree add -b "$BRANCH_NAME" "$BR_DIR" "host/$DEFAULT_BRANCH" || true',
    "  else",
    '    git -C /repo-shelf/repo worktree add -b "$BRANCH_NAME" "$BR_DIR" "origin/$DEFAULT_BRANCH" || true',
    "  fi",
    "fi",
